...
real(kind=8) :: a = 340.
    ...
    M_trnsl = -0.5
    om = -2.*pi/60. * 1000. !RPM
    alpha_wing = pi/180. * 4. !3.27

    !X,Y,Z VX,VY,VZ roll,pitch,yaw rollrate,pitchrate,yawrate
    if (n_trj == 1) then
      tvec(1:3) = traj_pos(n_trj, t, r_0)
!      modr = sqrt((r_0(1) - c_rot(1))*(r_0(1) - c_rot(1)) + (r_0(3) - c_rot(3))*(r_0(3) - c_rot(3)))
      tvec(4) = M_trnsl * a
      tvec(5:7) = 0.
      tvec(8) = alpha_wing
      tvec(9:12) = 0.
      ...
      tvec(4) = M_trnsl * a
      tvec(5) = -1.* modr * om * sin(om*t + acos( (r_0(2)-c_rot(2))/modr ) * sign( dble(1.), (r_0(3)-c_rot(3)) ))
      tvec(6) = modr * om * cos(om*t + acos( (r_0(2)-c_rot(2))/modr ) * sign( dble(1.), (r_0(3)-c_rot(3)) ))
      tvec(7) = om*t + acos( (r_0(2)-c_rot(2))/modr ) * sign( dble(1.), (r_0(3)-c_rot(3)) )
      tvec(8:9) = 0.
      tvec(10) = om
      tvec(11:12) = 0.
    end if
...
real(kind=8) :: a = 340.
    ...
    M_trnsl = -0.5
    om = -2.*pi/60. * 1000. !RPM
    alpha_wing = pi/180. * 3.27

    !X,Y,Z VX,VY,VZ roll,pitch,yaw rollrate,pitchrate,yawrate
    if (n_trj == 1) then
      c_rot(1) = 0. !-0.25 * 3.46 !This is at quarter root chord, based on input model
      c_rot(2) = 0.
      c_rot(3) = 0.
      ...
      if (modr > 0.) then
        ...
        traj_pos(1) = M_trnsl * a * t + c_rot(1)
        traj_pos(2) = r_0(2)
        traj_pos(3) = c_rot(3)
      end if
      ...
      modr = sqrt( (r_0(2)-c_rot(2))**2 + (r_0(3)-c_rot(3))**2 )
      if (modr == 0.) print*, 'Here'
      traj_pos(1) = r_0(1) + M_trnsl * a * t
      traj_pos(2) = modr * cos(om*t + acos( (r_0(2)-c_rot(2))/modr ) * sign( dble(1.), (r_0(3)-c_rot(3)) )) + c_rot(2)
      traj_pos(3) = modr * sin(om*t + acos( (r_0(2)-c_rot(2))/modr ) * sign( dble(1.), (r_0(3)-c_rot(3)) )) + c_rot(3)
    end if
